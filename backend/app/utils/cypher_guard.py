...
_WRITE_KEYWORDS = [
    ...
    "DROP", "ALTER", "GRANT", "REVOKE", "DETONATE",
]
...
def validate_cypher_params(params: dict) -> tuple[bool, Optional[str]]:
    """校验 Cypher 参数字典。
    ...
    return _validate_value(params, "")


def _validate_value(value, path: str) -> tuple[bool, Optional[str]]:
    if isinstance(value, dict):
        for k, v in value.items():
            if not isinstance(k, str) or not _SAFE_IDENTIFIER.match(k):
                return False, f"参数名格式无效: {path}{k}"
            ok, err = _validate_value(v, f"{path}{k}.")
            if not ok:
                return False, err
    elif isinstance(value, list):
        for i, v in enumerate(value):
            ok, err = _validate_value(v, f"{path}[{i}].")
            if not ok:
                return False, err
