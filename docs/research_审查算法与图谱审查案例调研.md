...
> 状态：已完成（调研结论已落入批次 7-10；2026-09-09 复核无剩余待办）
...
- [x] MMLogicInt 细节（已从公开摘要获得支撑设计所需的核心信息：多页 PDF 自动拆分识别、物流元数据提取、条目结构映射、图谱验证审计；无继续阻塞）
- [x] 昆仑数智合规方案（融合知识图谱+规则引擎：制度智能审查/合同全生命周期风险识别/风险预警闭环）
- [x] 天津大学国际工程合同问答（KG+LLM 双架构，DOI 10.1007/s42524-026-4237-0，626 份 88 国合同数据集）
- [x] 审查执行引擎设计模式（规则引擎 vs 图谱路径查询 vs 符号验证器）
- [x] 沉淀"可落地设计模式"清单 → 已转化为 task_plan 批次 7-10
