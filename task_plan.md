...
## Current Phase
无执行中的批次。批次 0-11 已完成；2026-09-09 已按当前仓库代码重新核验排期事项，最新有效清单见文末「2026-09-09 排期清单复核」。
...
4. ~~每批次是否需要用户确认后启动，还是授权连续执行？~~ → 已按文件化批次执行；仅高风险/外部动作需确认
5. ~~审查意图还原建模~~ → 已研究：条件/断言/例外 + 聚合/角色/Value 结构（批次 7-9）
6. ~~批次 7-9 是否需要与批次 1-2 穿插执行~~ → 批次 7-11 已完成，无需再决策
...
> 触发：系统崩溃后需要按当前代码重新核验，旧记录不再直接作为排期依据。
> 核验范围：当前工作区、npm/pnpm 锁文件、前端结果状态与规则编辑、后端审查服务、配置与依赖清单。

#### 保留待排期

| 优先级 | 事项 | 当前状态 |
|---|---|---|
| P1 | 后端依赖安全复核与升级 | `backend/requirements.txt` 仍保持 2026-08-13 的版本；旧 `pip-audit` 结论需要重新审计，不能直接沿用 |
| P1 | 生产化安全 | 当前仍无 API 鉴权/限流；`config.py` 仍保留默认 PG/Neo4j 口令兜底 |
| P2 | 统一前端包管理器与锁文件 | `pnpm-lock.yaml` 已锁定 `react-router 7.18.3`，但 `package-lock.json` 仍为 `7.18.1`，存在双锁文件漂移 |
| P2 | 审查结果状态流转 UI | 后端 `PATCH /api/reviews/results/{id}/status` 已存在；前端只有状态标签，没有打开/确认/修复/关闭操作按钮 |
| P2 | 规则 `structure` 编辑 | 前端类型已有 `structure` 字段，但规则编辑器不展示/维护 `condition`、`assertion`、`exceptions` |
| P3 | 可选远期能力 | 文档字段值写入 Neo4j 做真实 Cypher 聚合；Neo4j violation 持久化节点；普通多值聚合的语义兜底 |
| P1（长期核心） | 大篇幅制度文件与多模态来源的规则拆解 | 永久性核心课题：将自然语言制度文件、Excel、PDF、图片、关系图等来源正确拆解为符合图谱审查颗粒度的原子规则；详见下方专项设计卡 |

#### 专项设计卡：制度性文件与多模态来源的规则拆解

- **目标**：建立统一的“来源文档 → 结构解析 → 原子规则 → 图谱审查”链路，使任意来源的原始规则材料都能转化为可定位、可验证、可解释的详细规则，而不是依赖人工按“文件类型 × 检查项”补齐。
- **来源范围**：
  - 半结构化：Excel/CSV（已具备合并单元格展开与 `source_ref` 行级追溯基础）。
  - 长文本：PDF/DOCX/TXT/Markdown 制度文件、管理办法、操作规程、合同模板。
  - 图像：扫描件、截图、照片、表单图片（OCR + 版面结构 + 表格/字段识别）。
  - 关系图：流程图、组织图、审批链路图、业务关系图（实体/关系/条件抽取）。
  - 其他：后续出现的混合来源和自定义文件格式，通过统一适配器接入。
- **核心难点**：
  - **颗粒度判定**：区分“一条制度条款”与“一条可执行审查规则”，把复合句拆为原子规则，并补齐 `scope / condition / assertion / exceptions / tolerance / severity`。
  - **上下文消解**：处理章节/条款引用、代词、省略主语、跨段落条件、附件引用、定义术语和隐含例外。
  - **来源追溯**：每条规则必须回链到原文页码、段落、表格单元格、图片 bbox 或关系图元素，保留原文快照与解析依据。
  - **冲突与缺失检测**：识别重复、矛盾、边界重叠、条件不完整、无法核验和缺少关键字段的规则。
  - **多模态融合**：文本、表格、版面、图像和关系图需要统一成可推理的中间表示，不能只做全文 OCR 后直接丢给 LLM。
  - **泛化与稳定**：不同客户、不同文档风格、不同术语体系下保持拆解质量，避免规则被错误合并或漏拆。
- **建议分阶段实施**：
  1. **统一来源模型**：定义 `SourceDocument / SourceBlock / SourceLocator`，统一承载文本层级、表格行列、图片坐标和图结构。
  2. **长文本结构解析**：识别标题层级、条款编号、列表、表格、附件和交叉引用，先保结构再切块。
  3. **原子规则抽取**：采用“层级摘要 → 条款拆分 → 规则结构填充 → 原子性校验”的流水线，禁止因文本相似跨来源合并。
  4. **多模态适配器**：分别接入 Excel/表格、扫描 PDF/图片 OCR、关系图实体关系抽取；共用同一规则输出契约。
  5. **校验与人审闭环**：覆盖率、来源回链、冲突检测、低置信度标记，前端支持“原文 ↔ 原子规则”对照确认。
  6. **评测集**：按来源类型建立金标准样本，持续评估拆解覆盖率、原子性准确率、来源定位准确率、冲突召回率和人工修正率。
- **验收标准（首期）**：
  - 一份 20-50 页制度 PDF 能稳定拆出可审查规则，且每条规则都带来源定位和结构字段。
  - 同类规则不因表述差异被错误合并；复合条款能被拆成多条原子规则。
  - 图片/关系图至少能输出可追溯的中间结构，并明确哪些部分无法可靠解析。
  - 前端可查看原文与规则的一一对应关系，支持人工修正后重新入库。
  - 建立可重复运行的评测样本与指标，作为后续持续优化的基线。
- **状态**：长期排期，待专项设计；不阻塞当前已完成的图谱审查与规则导入能力。

#### 已移出排期清单

- 前端 `react-router` 漏洞升级：pnpm 锁文件已是 `7.18.3`；仅保留“统一锁文件”事项。
- ruff 清理：当前项目 venv 未安装 ruff，且不属于当前产品需求；恢复静态检查后再单独立项。
- 调研文档待办：`docs/research_审查算法与图谱审查案例调研.md` 的剩余未勾选项已由批次 7-10 落地，不再作为待排期项。

当前没有定义批次 12；GitHub 也没有未关闭的 Issue/PR。
